...
describe('computeBalance — grossDebts (un-netted directional)', () => {
  it('emits both directions when each side has paid for the other', () => {
    // u1 paid e1 (split 50/50) → u2 owes u1 50.
    // u2 paid e2 (split 50/50) → u1 owes u2 25.
    // Netted settlements would collapse to "u2 owes u1 25", losing the
    // breakdown of u1's debt to u2. grossDebts keeps both.
    const result = computeBalance({
      expenses: [
        expense({ id: 'e1', userId: 'u1', amount: 100, convertedAmount: 100, isSplit: true }),
        expense({ id: 'e2', userId: 'u2', amount: 50, convertedAmount: 50, isSplit: true }),
      ...
      splits: [
        split({ id: 's1', expenseId: 'e1', userId: 'u1', amount: 50, isPayer: true }),
        split({ id: 's2', expenseId: 'e1', userId: 'u2', amount: 50, isPayer: false }),
        split({ id: 's3', expenseId: 'e2', userId: 'u2', amount: 25, isPayer: true }),
        split({ id: 's4', expenseId: 'e2', userId: 'u1', amount: 25, isPayer: false }),
      ],
    });
    expect(result.grossDebts).toEqual([
      { fromUserId: 'u2', toUserId: 'u1', amount: 50 },
      { fromUserId: 'u1', toUserId: 'u2', amount: 25 },
    ]);
    // Netted settlements still collapse — that's the stats-card view.
    expect(result.settlements).toEqual([
      { fromUserId: 'u2', toUserId: 'u1', amount: 25 },
    ]);
  });

  it('is empty when no split expenses exist', () => {
    const result = computeBalance({
      expenses: [
        expense({ id: 'e1', userId: 'u1', amount: 100, convertedAmount: 100 }),
      ],
  ...
  it('reduces the matching direction when an unattributed settlement is recorded', () => {
    // u2 owes u1 50; u2 pays 30. Gross debt drops to 20, no reverse direction.
    const result = computeBalance({
      expenses: [
        expense({ id: 'e1', userId: 'u1', amount: 100, convertedAmount: 100, isSplit: true }),
      ],
  ...
  it('flips excess to the opposite direction on overpayment', () => {
    // u2 owes u1 50; u2 hands u1 70 (overpay by 20). Gross: u1 now owes u2 20.
    const result = computeBalance({
      expenses: [
        expense({ id: 'e1', userId: 'u1', amount: 100, convertedAmount: 100, isSplit: true }),
      ],
  ...
  it('drops zero-balance directions', () => {
    // u2 owes u1 50; u2 pays u1 50. Gross direction goes to 0 — not emitted.
    const result = computeBalance({
      expenses: [
        expense({ id: 'e1', userId: 'u1', amount: 100, convertedAmount: 100, isSplit: true }),
      ],
  ...
  it('keeps attributed-settled splits out of the gross', () => {
    // u2 owes 50 + 30 to u1 from two split expenses. Attributed settle wipes
    // the 50; gross should show only the remaining 30.
    const result = computeBalance({
      expenses: [
        expense({ id: 'e1', userId: 'u1', amount: 100, convertedAmount: 100, isSplit: true }),
        expense({ id: 'e2', userId: 'u1', amount: 60, convertedAmount: 60, isSplit: true }),
  ...
  it('sorts grossDebts by amount descending', () => {
    // Two independent pair-direction debts; the bigger one comes first.
    const result = computeBalance({
      expenses: [
        expense({ id: 'e1', userId: 'u1', amount: 200, convertedAmount: 200, isSplit: true }),
        expense({ id: 'e2', userId: 'u2', amount: 40, convertedAmount: 40, isSplit: true }),
      ],
      splits: [
        split({ id: 's1', expenseId: 'e1', userId: 'u1', amount: 100, isPayer: true }),
        split({ id: 's2', expenseId: 'e1', userId: 'u2', amount: 100, isPayer: false }),
        split({ id: 's3', expenseId: 'e2', userId: 'u2', amount: 20, isPayer: true }),
        split({ id: 's4', expenseId: 'e2', userId: 'u1', amount: 20, isPayer: false }),
      ],
    });
    expect(result.grossDebts.map((d) => d.amount)).toEqual([100, 20]);
  });
});
